...
**`HarnessProfile` registration is opt-in, and unregistered models are the normal case.** The one profile we register is keyed `anthropic:claude-sonnet-4-6`, so it applies to that model and nothing else. This is **not** a gap to close: a profile exists to *customize* behavior for a model that needs it, not to enable a model. An unregistered model runs with the framework defaults — general-purpose subagent enabled, no system-prompt suffix — and works fine. Only register a new profile when a specific provider or model misbehaves in a way worth correcting (e.g. it ignores skills unless told to read them first, which is exactly why sonnet has one). Do not add profiles preemptively for every provider.
