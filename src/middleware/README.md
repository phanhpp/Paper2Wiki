...
Makes wiki verification **mandatory** instead of optional.

`quick_wiki_integrity_check` is a *tool*, so the model may skip it and may ignore its failures — a
check the model can decline is a suggestion, not verification. This middleware moves the decision to
the harness: when a run finishes, work out what it did, check it, and if it's wrong send the agent
back with the specific problems to fix. Up to `max_iterations` times, then report and stop.

**No LLM call anywhere in the loop.** Every check reads files and compares strings. LLM-as-judge
grading (is the page faithful to the source paper?) stays in `eval/run_weekly_eval.py`, where the
whole paper can go in the prompt.
...
| `query` | …*and* the user's message asked for the wiki ("use the wiki", "what do we know about", "check our notes") |
...
A run can match more than one ("ingest this paper and make slides" is both), and all matching checks
run.
...
Take a photo of the wiki before, take another after, compare.

**Before the agent starts**, list every file with its size and last-modified time:
...
**Agent runs.** Writes whatever it writes.

**After it finishes**, list everything again:
...
Compare the two lists. `index.md`, `log.md`, `concepts/attention.md` → those are this run's writes.

**Why size *and* time:** `log.md` gets appended to. Two appends in the same second would show the same
timestamp and look untouched. The size changes, so we catch it.

**Why not just watch the tool calls:** the agent writes files by more routes than `write_file` — shell
commands, the sandbox download tool, and whatever gets added later. None of those show up as a
`write_file` call. Looking at the files on disk catches every write, however it was made, and needs no
update when a new tool appears.

### Two photos, not one

`marp-slides/` is a **sibling** of `wiki/`, not inside it, so a walk of `wiki/` never reaches it:

```
llm_wiki/
├── wiki/           ← wiki_snapshot      → RunContext.writes     (ingest, query checks)
└── marp-slides/    ← artifact_snapshot  → RunContext.artifacts  (marp checks)
```

Slides land there because the marp subagent builds them in the Daytona sandbox and downloads them to
the host. With only a `wiki/` snapshot, a deck produces zero detected writes — the marp path never
fires and decks go unchecked.
...
Two semantic checks are designed but absent, because both need an embedding index: whether the pages
the agent read were actually relevant to the question, and whether the wiki held relevant pages the
agent ignored.
...
2. Append it to that module's `CHECKS` list.
3. Add a test with a seeded defect in `tests/test_wiki_checks.py`.
